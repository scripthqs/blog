...
![标准盒模型](../images/content_box.jpeg)
![IE盒模型](../images/border_box.jpeg)

- **标准盒子模型**中，width 和 height 指的是**内容区域 content** 的宽度和高度。
- **IE盒子模型**（怪异盒模型）中，width 和 height 指的是**内容区域 content + border + padding**的宽度和高度。
...
border 边框属性可以设置元素的**边框**，border 由3部分组成：**边框宽度**、**边框样式**、**边框颜色**。
...
padding 内边距属性可以设置**内边距**，即边框和内容的距离 。padding 有4个方向。
