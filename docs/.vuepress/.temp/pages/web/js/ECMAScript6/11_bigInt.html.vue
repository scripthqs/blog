<template><div><h1 id="bigint" tabindex="-1"><a class="header-anchor" href="#bigint" aria-hidden="true">#</a> BigInt</h1>
<p>ES5 之前，只有 6 种数据类型，ES6 之后，到目前为止（2021），JavaScript 一共有 8 种数据类型。</p>
<p>其中，基本数据类型</p>
<ul>
...
<p>引用数据类型</p>
<ul>
<li><strong><code v-pre>Object</code> 对象</strong></li>
</ul>
<p>BigInt 数据类型提供了一种方法来表示大于 2^53-1 的整数。BigInt 可以<strong>表示任意大的整数</strong>。</p>
<h2 id="作用" tabindex="-1"><a class="header-anchor" href="#作用" aria-hidden="true">#</a> 作用</h2>
<p>Number 类型只能安全的支持-9007199254740991(-(2^53-1)) 和 9007199254740991(2^53-1)之间的整数，任何超过这个范围的数值都会失去精度；而 BigInt 可以解决这个问题</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上图，当数值超过 Number 数据类型支持的安全范围值时，将会被四舍五入，从而导致精度缺失的问题</p>
...
<ol>
<li>
<p>BigInt 除了不能使用一元加号运算符外，其他的运算符都可以使用</p>
...
<li>
<p>BigInt 和 Number 之间不能进行混合操作</p>
