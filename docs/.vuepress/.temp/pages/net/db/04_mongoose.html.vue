<template><div><h1 id="mongoose" tabindex="-1"><a class="header-anchor" href="#mongoose" aria-hidden="true">#</a> Mongoose</h1>
<h2 id="mongoose-介绍" tabindex="-1"><a class="header-anchor" href="#mongoose-介绍" aria-hidden="true">#</a> Mongoose 介绍</h2>
<p>网址：<a href="http://www.mongoosejs.net/docs/index.html" target="_blank" rel="noopener noreferrer">http://www.mongoosejs.net/docs/index.html<ExternalLinkIcon/></a></p>
<p>mongoose 是 Node 环境下异步操作 mongodb 数据库的扩展，仅限于 Node 环境下使用。</p>
<p>在实际开发中，通过命令管理 mongodb 麻烦，通过程序管理。</p>
<p>使用 mongoose 操作 mongodb 数据步骤：</p>
<ol>
<li>使用 npm 安装 mongoose</li>
<li>导入模块</li>
<li>然后使用</li>
<li>连接 mongodb 数据库</li>
<li>定义 Schema
<ul>
<li>schema：简单的来讲就是规定了表结构的操作。比如：要求表中有 username 字段，该字段接着要求必填，可以继续要求该字段类型为 string，还可以要求这个字段长度为 10（相当于 mysql 建表时候的 sql 语句对字段的规定）</li>
</ul>
</li>
<li>定义 model
<ul>
<li><strong>mvc：开发模式</strong>，要求将一个项目拆分成三个大的主体部分（m 模型、v 视图、c 控制器），<strong>==其中 m 负责业务逻辑与数据库的交互部分==、v 负责展示给用户页面、c 控制器负责请求与响应的整体调度</strong>。</li>
</ul>
</li>
<li>使用 model 进行数据增删改查操作</li>
</ol>
<h2 id="连接数据库" tabindex="-1"><a class="header-anchor" href="#连接数据库" aria-hidden="true">#</a> 连接数据库</h2>
<p>使用 npm 安装 mongoose 模块，并在使用模块中导入：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 安装mongoose</span>
<span class="token function">npm</span> i <span class="token parameter variable">-S</span> mongoose
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// 导入模块</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="定义-schema" tabindex="-1"><a class="header-anchor" href="#定义-schema" aria-hidden="true">#</a> 定义 Schema</h2>
<p>Schema 是 mongoose 中会用到的一种数据模式，可以理解为数据表结构的定义；每个 schema 会映射到 mongodb 中的一个集合，它不具备操作数据库的能力。Schema 中定义数据校验，默认值，字段名，字段类型等特性。</p>
<p>作用：</p>
<ul>
<li>建表（也就是说，后期建表不再通过 mongoDB 的命令行的形式产生了，而是通过 JavaScript 代码实现）</li>
</ul>
<p>在定义 schema 的是有使用到相关约束规则，可以查看：<a href="http://www.mongoosejs.net/docs/guide.html" target="_blank" rel="noopener noreferrer">http://www.mongoosejs.net/docs/guide.html<ExternalLinkIcon/></a></p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// 创建用户集合规则</span>
...
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在 Schema 定义好后，后期增删改查时，如果对应的表不存在，则其会自动按照 Schema 的约束进行建表，如果表存在了则不重新建立。</p>
<h2 id="定义-model" tabindex="-1"><a class="header-anchor" href="#定义-model" aria-hidden="true">#</a> 定义 model</h2>
<p>model 是由 schema 生成的模型，模型是最终用来进行数据增删改查操作使用的，可以对数据库的操作：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// 参数1：model名称</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="在-express-中使用-mongoose" tabindex="-1"><a class="header-anchor" href="#在-express-中使用-mongoose" aria-hidden="true">#</a> 在 express 中使用 mongoose</h2>
<p>需求：通过 postman 发送 post 数据给 express 服务器，能够对 mongoDB 中的<code v-pre>shop</code>库中的<code v-pre>members</code>表进行插入操作。</p>
<ul>
<li>创建库
<ul>
<li><code v-pre>use shop</code></li>
</ul>
</li>
<li>创建 express 服务器，接受 post 传参</li>
<li>连接数据库（mongoDB）</li>
<li>定义 schema（创建表<code v-pre>members</code>）</li>
<li>定义模型实现数据的添加操作</li>
</ul>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// - 创建express服务器，接受post传参</span>
