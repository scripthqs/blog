<template><div><h1 id="组合式-api" tabindex="-1"><a class="header-anchor" href="#组合式-api" aria-hidden="true">#</a> 组合式 API</h1>
<p>Composition API：组合式 API</p>
<p>官方文档: <a href="https://v3.cn.vuejs.org/guide/composition-api-introduction.html" target="_blank" rel="noopener noreferrer">https://v3.cn.vuejs.org/guide/composition-api-introduction.html<ExternalLinkIcon/></a></p>
<h2 id="options-api-和-composition-api" tabindex="-1"><a class="header-anchor" href="#options-api-和-composition-api" aria-hidden="true">#</a> Options API 和 Composition API</h2>
<h3 id="options-api-存在的问题" tabindex="-1"><a class="header-anchor" href="#options-api-存在的问题" aria-hidden="true">#</a> Options API 存在的问题</h3>
<p>使用传统 OptionsAPI 中，新增或者修改一个需求，就需要分别在 data，methods，computed 里修改。</p>
<h3 id="composition-api-的优势" tabindex="-1"><a class="header-anchor" href="#composition-api-的优势" aria-hidden="true">#</a> Composition API 的优势</h3>
...
<ol>
<li>
<p>理解：Vue3.0 中一个新的配置项，值为一个函数。</p>
</li>
<li>
<p>setup 是所有<strong>Composition API（组合 API）</strong> “<em>表演的舞台</em> ”</p>
</li>
<li>
<p>组件中所用到的：<strong>数据、方法</strong>等等，均要<strong>配置在 setup</strong>中。</p>
</li>
<li>
<p>setup 函数的两种返回值</p>
</li>
<li>
<p>若<strong>返回一个对象</strong>，则对象中的属性、方法, 在模板中均可以直接使用。（重点关注！）</p>
...
<li>
<p><em>若返回一个渲染函数：则可以自定义渲染内容。（<strong>了解</strong>）</em></p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> h <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">"vue"</span><span class="token punctuation">;</span>
...
<p>注意:</p>
<p>尽量<strong>不要与 Vue2.x 配置混用</strong></p>
<ul>
<li>Vue2.x 配置（data、methods、computed...）中<strong>可以访问到</strong>setup 中的属性、方法。</li>
<li>但在 setup 中<strong>不能访问到</strong>Vue2.x 配置（data、methods、computed...）。</li>
<li>如果有重名, <strong>setup 优先</strong>。</li>
</ul>
<p>setup<strong>不能是一个 async 函数</strong>，因为返回值不再是 return 的对象，而是 promise，模板看不到 return 对象中的属性。（后期也可以返回一个 Promise 实例，但需要 Suspense 和异步组件的配合）</p>
<h2 id="ref-函数" tabindex="-1"><a class="header-anchor" href="#ref-函数" aria-hidden="true">#</a> ref 函数</h2>
<p>作用: 定义一个响应式的数据</p>
<p>需要先引入</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> ref <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">"vue"</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>语法:</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">let</span> xxx <span class="token operator">=</span> <span class="token function">ref</span><span class="token punctuation">(</span>initValue<span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<li>JS 中操作（改）数据： <code v-pre>xxx.value</code></li>
<li>模板中读取数据: 不需要使用.value</li>
</ul>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>ref 函数接收的数据类型：</p>
<ul>
<li>接收的数据可以是：基本类型、也可以是对象类型。</li>
<li>基本类型的数据：响应式依然是靠<code v-pre>Object.defineProperty()</code>的<code v-pre>get</code>与<code v-pre>set</code>完成的。</li>
<li>对象类型的数据：内部 “ 求助 ”了 Vue3.0 中的一个新函数—— <code v-pre>reactive</code>函数。</li>
</ul>
<h2 id="reactive-函数" tabindex="-1"><a class="header-anchor" href="#reactive-函数" aria-hidden="true">#</a> reactive 函数</h2>
<p>作用: 定义一个<strong>对象类型</strong>的响应式数据（基本类型不要用它，要用<code v-pre>ref</code>函数）</p>
<p>需要先引入</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">import</span> <span class="token punctuation">{</span> reactive <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">"vue"</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>语法：</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> 代理对象 <span class="token operator">=</span> <span class="token function">reactive</span><span class="token punctuation">(</span>源对象<span class="token punctuation">)</span><span class="token punctuation">;</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>接收一个对象（或数组），返回一个<strong>代理对象（Proxy 的实例对象，简称 proxy 对象）</strong></p>
<p>reactive 定义的响应式数据是“<strong>深层次的</strong>”</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token literal-property property">hobby</span><span class="token operator">:</span> <span class="token punctuation">[</span><span class="token string">"抽烟"</span><span class="token punctuation">,</span> <span class="token string">"喝酒"</span><span class="token punctuation">,</span> <span class="token string">"烫头"</span><span class="token punctuation">]</span><span class="token punctuation">;</span>
...
<p>直接通过<strong>修改索引改数组，也是响应式</strong>，vue2.0 通过索引修改数组不是响应式</p>
</blockquote>
<p>内部<strong>基于 ES6 的 Proxy 实现</strong>，通过代理对象操作源对象内部数据进行操作。</p>
<h2 id="vue3-0-的响应式原理" tabindex="-1"><a class="header-anchor" href="#vue3-0-的响应式原理" aria-hidden="true">#</a> Vue3.0 的响应式原理</h2>
<h3 id="vue2-x-的响应式" tabindex="-1"><a class="header-anchor" href="#vue2-x-的响应式" aria-hidden="true">#</a> vue2.x 的响应式</h3>
<ul>
<li>
<p>实现原理：</p>
<ul>
...
<li>
<p>数组类型：通过<strong>重写更新数组的一系列方法</strong>来实现拦截。（对数组的变更方法进行了包裹）。</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code>Object<span class="token punctuation">.</span><span class="token function">defineProperty</span><span class="token punctuation">(</span>data<span class="token punctuation">,</span> <span class="token string">"count"</span><span class="token punctuation">,</span> <span class="token punctuation">{</span>
...
<p>存在问题：</p>
<ul>
<li><strong>新增属性、删除属性</strong>, 界面不会更新。</li>
<li>直接通过<strong>下标修改数组</strong>, 界面不会自动更新。</li>
<li>通过 Vue.set()方法或者 <code v-pre>this.$set()</code>方法修改，解决问题</li>
<li>通过 Vue.delete()或者 <code v-pre>this.$delete()</code>方法删除，解决问题</li>
<li>使用数组的 splice 方法解决</li>
</ul>
</li>
</ul>
<h3 id="vue3-0-的响应式" tabindex="-1"><a class="header-anchor" href="#vue3-0-的响应式" aria-hidden="true">#</a> Vue3.0 的响应式</h3>
<ul>
<li>
<p>实现原理:</p>
<ul>
<li>
<p>通过 Proxy（代理）: <strong>拦截对象中任意属性的变化</strong>, 包括：属性值的读写、属性的添加、属性的删除等。</p>
</li>
<li>
<p>通过 Reflect（反射）: 对源对象的属性进行操作。</p>
</li>
<li>
<p>MDN 文档中描述的 Proxy 与 Reflect：</p>
<ul>
<li>
<p>Proxy：<a href="https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Proxy" target="_blank" rel="noopener noreferrer">https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Proxy<ExternalLinkIcon/></a></p>
</li>
<li>
<p>Reflect：<a href="https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Reflect" target="_blank" rel="noopener noreferrer">https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Reflect<ExternalLinkIcon/></a></p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token keyword">new</span> <span class="token class-name">Proxy</span><span class="token punctuation">(</span>data<span class="token punctuation">,</span> <span class="token punctuation">{</span>
...
<li>备注：ref 也可以用来<strong>定义对象（或数组）类型数据</strong>, 它内部会自动通过<code v-pre>reactive</code>转为<strong>代理对象</strong>。</li>
</ul>
<p>从原理角度对比：</p>
<ul>
<li>ref 通过<code v-pre>Object.defineProperty()</code>的<code v-pre>get</code>与<code v-pre>set</code>来实现响应式（数据劫持）。</li>
<li>reactive 通过使用<strong>Proxy</strong>来实现响应式（数据劫持）, 并通过<strong>Reflect</strong>操作<strong>源对象</strong>内部的数据。</li>
</ul>
<p>从使用角度对比：</p>
<ul>
<li>ref 定义的数据：操作数据<strong>需要</strong><code v-pre>.value</code>，读取数据时模板中直接读取<strong>不需要</strong><code v-pre>.value</code>。</li>
<li>reactive 定义的数据：操作数据与读取数据：<strong>均不需要</strong><code v-pre>.value</code>。</li>
</ul>
<h2 id="setup-的两个注意点" tabindex="-1"><a class="header-anchor" href="#setup-的两个注意点" aria-hidden="true">#</a> setup 的两个注意点</h2>
<ul>
<li>
<p>setup 执行的时机</p>
<ul>
...
<li>context：上下文对象
<ul>
<li>attrs: 值为对象，包含：组件外部传递过来，但<strong>没有在 props 配置中声明的属性</strong>, 相当于 <code v-pre>this.$attrs</code>。</li>
