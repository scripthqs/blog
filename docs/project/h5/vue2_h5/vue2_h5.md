...
在 app.vue 文件中，引入**Tabbar 底部切换**的标签栏组件，然后再**配置路由**的映射关系
...
5. 定义 getters，其实就是相当于 vue 里面的计算属性，将 state 中的数据进行计算处理
...
解决移动端点击 300ms 延迟问题，引入 FastClick，插件使用的步骤：下载-引入-配置
